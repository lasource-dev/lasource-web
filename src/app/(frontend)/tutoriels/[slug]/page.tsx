...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const content = await loadPublishedEditorialContent(slug, 'tutorial', async () => {
    ...
    const result = await payload.find({
      ...
      pagination: false,
      where: { and: [{ slug: { equals: slug } }, { content_type: { equals: 'tutorial' } }] },
    })
    return result.docs
  })
  return content
...
export default async function TutorialPage({ params }: PageProps) {
  const { slug } = await params
  return <EditorialContentPage slug={slug} type="tutorial" />
}
